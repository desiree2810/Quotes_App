...
const separator = import.meta.env.VITE_API_SEPARATER || ";";
...
const quoteService = {
  ...
  removeLikeFromQuote: async (quoteId, token) => {
    try {
      const headers = getHeaders(token);

      const response = await axios.patch(
        `${baseURL}/quotes/${quoteId}/like/down`,
        {},
        { headers }
      );
      return response;
  ...
  removeDislikeFromQuote: async (quoteId, token) => {
    try {
      const headers = getHeaders(token);

      const response = await axios.patch(
        `${baseURL}/quotes/${quoteId}/dislike/down`,
        {},
        { headers }
      );
      return response;
  ...
  getAllDislikedUsersList: async (quote, token) => {
    try {
      ...
      const response = await axios.get(
        `${baseURL}/quotes/${quote.id}/dislike/users`,
        { headers }
      );
      return response.data.users;
    } catch (error) {
  ...
  addQuote: async (quote, author, tags, token) => {
    try {
      ...
      const response = await axios.post(
        `${baseURL}/quotes`,
        { quote, author, tag: tags.join(separator) },
        { headers }
      );
      return response.data;
    } catch (error) {
